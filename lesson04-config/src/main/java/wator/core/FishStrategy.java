...
/**
 * This strategy handles the life of a fish. It's
 * <a href="https://en.wikipedia.org/wiki/Wa-Tor">rules</a> are:
 * <ol>
 * <li>At each chronon, a fish moves randomly to one of the adjacent unoccupied
 * squares. If there are no free squares, no movement takes place.</li>
 * <li>Once a fish has survived a certain number of chronons it may reproduce.
 * This is done as it moves to a neighbouring square, leaving behind a new fish
 * in its old position. Its reproduction time is also reset to zero.</li>
 * </ol>
 * This class separates the logic from the domain object. Hence it is easy to
 * replace that logic.
 */
@Named
public class FishStrategy {
    ...
    /**
     * Simulates the life of a fish for one chronon. It must processes only those
     * fish it is responsible for.
     *
    ...
    /**
     * Either increases the breed count, or if the breed count has reached the breed
     * time, then create a child in an empty neighbor cell, if there is any, and
     * reset the breed count to zero.
     */
    ...
    protected void move(Fish fish) {
        ...
        if (emptyNeighborCell.isPresent()) {
            ...
            // Write the fish to the torus even if it has not been moved. This is required
            // to update the properties
            // of a fish in the torus. I. e. a graphical torus may encode the properties of
            // a fish in the color of
            // a pixel. Thus if the properties of a fish changed the color if the pixel has
            // to be updated.
    ...
    /**
     * Add a new fish on the torus besides the current fish on the side given by the
     * direction.
     *
     * @param fish      the parent
     * @param direction specifies the the side to add the child relative to it's
     *                  parent
    ...
    /**
     * Find a randomly picked neighbor cell with properties specified by some
     * predicate.
     *
    ...
    /**
     * Returns the neighbor of the fish in the corresponding direction or null if
     * there is none.
     */
    ...
    protected int breedTime() {
        return settings.getFishBreedTime();
    }
